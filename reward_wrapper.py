...
class RewardShapingWrapper(gym.Wrapper):
    def __init__(self, env, ttc_threshold: float = 3.0, ttc_weight: float = 0.1,
        ...
        llm_judge_fn: optional callable(stats_dict) -> 0 or 1, e.g.
                      llm_judge.judge_episode. Called once at the end of an
                      episode (not every step - see llm_judge.py for why).
                      If None, no LLM judging happens (Phase 0 behavior).
    ...
    def step(self, action):
        ...
        # exposed for logging/diagnostics in train.py - does not affect the
        ...
        if done and self.llm_judge_fn is not None:
            ...
            if self._episode_count % self.llm_judge_every_n_episodes == 0:
                ...
                score = self.llm_judge_fn(episode_stats)  # blocking call - see llm_judge.py
