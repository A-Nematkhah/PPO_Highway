...
# will be used later by networks.py / ppo.py / train.py
N_STEPS = 128
BATCH_SIZE = 64
N_EPOCHS = 10
...
# --------------------------------------------------------------------------- #
# Phase 1 LLM integration (llm_judge.py) - an LLM judges each finished
# episode and returns a normalized 0/1 score, added to the terminal reward.
# Off by default: requires `pip install groq` + a GROQ_API_KEY, and slows
# training noticeably (each judged episode = one blocking network call).
# --------------------------------------------------------------------------- #
USE_LLM_JUDGE = False
...
GROQ_MODEL = "openai/gpt-oss-20b"  # llama-3.1-8b-instant was deprecated by Groq
                                    # (June 2026); this is their official
                                    # recommended replacement for fast/simple tasks
