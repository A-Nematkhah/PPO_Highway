...
3. **Per-step calls** (`eureka/candidate_wrapper.py`) — `shaping_reward()` wrapped with a wall-clock timeout (`SHAPING_FN_TIMEOUT_S`) via a module-level `ThreadPoolExecutor` (`SHAPING_FN_EXECUTOR_WORKERS`, default **8**).
...
| Hang after many steps | Per-step timeout in `shaping_call.py` + leak detection / executor replacement when all workers are saturated |
...
| Timed-out shaping thread keeps running | **Partial** — CPython cannot kill a hung worker thread. Timed-out futures stay "leaked" until they finish. `shaping_call.py` counts outstanding leaks, logs `shaping_call_timeout` / `shaping_executor_replaced`, and swaps in a fresh pool (default size 8) so the process does not permanently degrade to `(0.0, {})` shaping | Phase 3: subprocess-per-call or DSL (no `exec`) |
